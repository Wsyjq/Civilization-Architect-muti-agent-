...
export default function EditorPage() {
  ...
  const handleRunRound = async () => {
    ...
    setDisplayedMessages([]) // 清空当前显示的消息
    ...
    try {
      ...
      if (error.response?.status === 404) {
        const errorMsg = error.response?.data?.detail || '游戏会话不存在或已结束'
        alert(`${errorMsg}，将返回首页重新开始游戏。`)
        navigate('/')
      }
    } finally {
      setIsRunning(false)
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      {/* 顶部导航栏 */}
      <header className="flex-shrink-0 glass border-b border-cyber-border px-4 py-3 z-40">
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0">
        {/* 左侧：架构图 */}
        <div className="md:w-1/2 p-4 overflow-auto flex-shrink-0">
        ...
        {/* 右侧：聊天区域 - 固定高度，内部可滚动 */}
        <div className="md:w-1/2 flex flex-col p-4 overflow-hidden min-h-0">
          <div className="card flex-1 flex flex-col overflow-hidden min-h-0">
            ...
            <div className="flex-1 overflow-y-auto space-y-2 mb-4 min-h-0 pr-1">
              ...
              {isThinking && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="flex items-center gap-3 p-4 rounded-lg bg-cyber-secondary/30"
                >
                  <motion.div
                    animate={{ rotate: 360 }}
                  ...
                  <div className="flex items-center gap-1">
                    <span className="text-cyber-text-muted">Agent正在思考</span>
                    <motion.span
            ...
            <div className="flex gap-3 flex-shrink-0">
              ...
              >
                ...
                {isThinking ? '思考中...' : isRunning ? '模拟中...' : '执行本轮'}
              </button>
              <button
                onClick={handleRunRound}
                disabled={isRunning}
      ...
      {/* 底部：Agent卡片池 - 固定高度 */}
      <div className="flex-shrink-0 border-t border-cyber-border">
        <button
          onClick={() => setShowCardPool(!showCardPool)}
          className="w-full px-4 py-2 flex items-center justify-center gap-2 text-cyber-text-muted hover:text-cyber-accent transition-colors hover:bg-cyber-secondary/30"
        >
...
function AgentMiniCard({ agent, isPlaced }: { agent: Agent; isPlaced: boolean }) {
  // 八维性格完整展示
  const allTraits = [
    { key: '权威', value: agent.personality.authority },
    ...
    { key: '风险', value: agent.personality.risk_appetite },
    { key: '智力', value: agent.personality.intelligence },
    { key: '忠诚', value: agent.personality.loyalty_base },
  ]

  return (
    <div
      className={`flex-shrink-0 w-24 p-2 rounded-lg border transition-all cursor-pointer ${
        isPlaced
    ...
    >
      <div className="flex flex-col items-center">
        ...
        {/* 八维属性 */}
        {!isPlaced && (
          <div className="mt-2 w-full grid grid-cols-4 gap-0.5">
            {allTraits.map(({ key, value }) => (
              <div key={key} className="flex flex-col items-center">
                <div className="w-full h-0.5 bg-cyber-border rounded-full overflow-hidden">
                  <div
                    className="h-full bg-cyber-accent rounded-full"
                    style={{ width: `${value * 100}%` }}
                  />
                </div>
                <span className="text-[8px] font-mono text-cyber-accent">{(value * 100).toFixed(0)}</span>
              </div>
            ))}
          </div>
        )}
