...
管理游戏循环、状态更新、产出结算
支持两种模式：
1. 模拟模式（默认）：使用规则引擎计算，适合快速测试
2. 多Agent模式：使用LLM驱动的真实Agent交互
"""
...
# 多Agent支持
try:
    from backend.multi_agent.coordinator import Coordinator, CycleResult
    from backend.multi_agent.message_broker import reset_message_broker
...
class GameEngine:
    ...
    支持两种模式：
    - simulation_mode=True: 使用规则引擎（快速，适合测试）
    - simulation_mode=False: 使用LLM驱动的多Agent（真实交互）
    """
    ...
    def __init__(self, num_civilizations: int = None,
        ...
        Args:
            ...
            seed: 随机种子
            config: 游戏配置
            simulation_mode: True=规则模拟, False=LLM多Agent
        """
        # 使用配置或默认值
    ...
    async def run_round(self, civilization: Civilization):
        """执行一个回合"""
        if not self.simulation_mode and MULTI_AGENT_AVAILABLE:
            # 多Agent模式
            await self._run_round_multi_agent(civilization)
        else:
            # 模拟模式（原逻辑）
            cycles = civilization.config.cycles_per_round
            for cycle in range(cycles):
                civilization.state.cycle += 1
                self._run_cycle_simulation(civilization)

            civilization.state.round += 1
            civilization.state.cycle = 0

    async def _run_round_multi_agent(self, civilization: Civilization):
        """多Agent模式执行回合"""
        if civilization.civilization_id not in self._coordinators:
            # 创建协作者
            coordinator = Coordinator(
                agents=civilization.agents,
                adjacency_matrix=civilization.config.adjacency_matrix,
                architecture_type=civilization.architecture_type.value,
                config=self.config
            )
            self._coordinators[civilization.civilization_id] = coordinator

        coordinator = self._coordinators[civilization.civilization_id]
        coordinator.round_num = civilization.state.round
        coordinator.total_rounds = self.total_rounds
        ...
        for _ in range(cycles):
            ...
            # 更新文明状态
            civilization.state.total_output = coordinator.total_output
            civilization.state.cycle_outputs.append(result.total_output)
            civilization.state.energy_level_history.append(result.macro_variables["energy_level"])
            civilization.state.cohesion_history.append(result.macro_variables["cohesion"])
            civilization.state.fidelity_history.append(result.macro_variables["fidelity"])
            civilization.state.social_capital_history.append(result.macro_variables["social_capital"])
            civilization.state.cycle = result.cycle_num

        civilization.state.round += 1
        civilization.state.cycle = 0

    def _run_cycle_simulation(self, civilization: Civilization):
        """模拟模式执行循环（原run_cycle逻辑）"""
        agents = civilization.agents
    ...
    async def run_full_game(self) -> Dict:
        """运行完整游戏"""
