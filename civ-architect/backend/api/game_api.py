...
class StartGameRequest(BaseModel):
    ...
    simulation_mode: bool = True  # True=规则模拟, False=LLM多Agent
...
async def run_round(game_id: str, request: RunRoundRequest = None):
    ...
    # 执行一轮
    await session.engine.run_round(session.civilization)
...
async def configure_llm(request: LLMConfigRequest):
    ...
    配置用于多Agent模式的LLM后端
    支持: deepseek (默认), openai, anthropic, ollama
    """
    try:
        from backend.core.llm_config import LLMConfig, configure_llm
        ...
        return {"success": True, "message": f"LLM已配置为 {request.provider}/{request.model}"}
    except ImportError:
        raise HTTPException(status_code=500, detail="多Agent模块未安装")


@router.get("/config/llm", response_model=LLMStatusResponse)
async def get_llm_status():
    ...
    try:
        ...
        from backend.multi_agent import coordinator as ma_module

        manager = get_llm_manager()
        return LLMStatusResponse(
            configured=manager.config.api_key is not None,
...
async def test_llm_connection():
    ...
    """
    try:
        from backend.core.llm_config import test_llm_connection as _test

        result = await _test()
        if result:
            return {"success": True, "message": "LLM连接正常"}
        else:
            return {"success": False, "message": "LLM连接失败"}
    except ImportError:
        raise HTTPException(status_code=500, detail="多Agent模块未安装")
