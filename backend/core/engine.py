...
class GameEngine:
    ...
    def _generate_agent_communications(self, civilization: Civilization):
        ...
        try:
            # 设置当前回合和循环
            civilization.message_generator.set_round_cycle(
                civilization.state.round,
                civilization.state.cycle
            )
            
            # 准备近期事件
            recent_events = []
            if civilization.state.cycle_outputs:
                last_output = civilization.state.cycle_outputs[-1]
                recent_events.append(f"上轮产出: {last_output:.1f}星尘")
            
            # 检查内鬼活动
            traitor_count = sum(1 for a in civilization.agents if a.is_active_traitor)
            if traitor_count > 0:
                recent_events.append(f"发现{traitor_count}个可疑人员")
            
            # 生成通讯
            messages = civilization.message_generator.generate_communications(
                agents=civilization.agents,
            ...
            if messages:
                print(f"[{civilization.civilization_id}] 循环{civilization.state.cycle}: 生成了{len(messages)}条消息")
                
        except Exception as e:
            print(f"生成Agent对话时出错: {e}")
            import traceback
            traceback.print_exc()
