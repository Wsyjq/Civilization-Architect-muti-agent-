...
如果遇到 `429 Client Error: Too Many Requests`，说明API调用频率过高。系统已内置以下保护机制：

1. **限流控制**: 自动限制每分钟请求数（默认10次/分钟）
2. **指数退避重试**: 遇到429错误时自动等待并重试（最多3次）
3. **并发控制**: 限制同时进行的API调用数量（默认3个）
4. **降级机制**: 达到最大重试次数后自动使用模拟回复

**解决方案**:
- 降低 `LLM_RATE_LIMIT_REQUESTS`（建议免费套餐设为 3-5）
- 增加 `LLM_RETRY_DELAY`（建议设为 2.0 或更高）
- 降低 `LLM_MAX_CONCURRENT`（建议设为 1-2）
- 在游戏配置中减少每轮循环的消息数量
